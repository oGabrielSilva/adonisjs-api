...
export default class GroupRequestsController {
  public async index({ request, response }: HttpContextContract) {
    ...
    const groupRequests = await GroupRequest.query()
      ...
      .preload('group', (query) => query.select('name', 'master'))
      .preload('user', (query) => query.select('username'))
      .whereHas('group', (query) => query.where('master', Number(master)))
  ...
  public async store({ request, response, auth }: HttpContextContract) {
    const groupId = request.param('id') as number
    ...
    const userAlreadyInGroup = await Group.query()
      .whereHas('players', (query) => query.where('id', userId))
      .andWhere('id', groupId)
      .first()

    if (userAlreadyInGroup) throw new BadRequestException('user already in the group', 422)
  ...
  public async update({ request, response, bouncer }: HttpContextContract) {
    ...
    const groupForUpdate = await Group.findOrFail(id)
    await bouncer.authorize('updateGroup', groupForUpdate)
    const group = await groupForUpdate.merge(payload).save()
